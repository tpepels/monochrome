...
- `docker/docker-compose.yml` exposes download runtime settings, upstream provider settings with upstream-matching defaults, mounts the music library, and includes an optional Redis profile. The upstream Amazon Turnstile path is guarded for non-browser server runtimes so it can fall through without a `document` error when no bypass token is configured.
