...
export class MonochromeResolverFacade {
    constructor({ env = {}, tracksApi = null } = {}) {
        this.env = env;
        this.tracksApi =
            tracksApi ||
            new TracksStreamerAPI(String(env.TRACKS_API_BASE_URL || TRACKS_API_BASE_URL).replace(/\/+$/, ''));
    }

    async resolveTrackDownload(trackId, quality = 'LOSSLESS', { track = null } = {}) {
        const normalizedQuality = normalizeQuality(quality);
        const metadata = normalizeTrack(track || { id: trackId });
        const stream = await this.tracksApi.resolveTrackStream(trackId, normalizedQuality, { track: metadata });

        if (!stream?.url) {
            const error = new Error(`Could not resolve audio stream for track ID: ${trackId}`);
    ...
    async resolveAlbum(albumId, { album = null, tracks = null } = {}) {
        ...
        const normalizedAlbum = normalizeAlbum(resolvedAlbum);
        const normalizedTracks = (rawTracks || []).map((track, index) => ({
            ...normalizeTrack(track, normalizedAlbum),
        ...
        return {
            ...
            provider: 'monochrome',
            providerInstance: null,
            id: String(normalizedAlbum?.id ?? albumId),
            metadata: normalizedAlbum,
...
export class ServerResolverAdapter {
    ...
    async resolveAlbum(albumId, options = {}) {
        return this.facade.resolveAlbum(albumId, options);
    }
}
...
export async function resolveTrackDownload(trackId, quality = 'LOSSLESS', options = {}) {
    const { track = null, ...adapterOptions } = options;
    return createResolverAdapter(adapterOptions).resolveTrackDownload(trackId, quality, { track });
}

export async function resolveAlbum(albumId, options = {}) {
    const { album = null, tracks = null, ...adapterOptions } = options;
    return createResolverAdapter(adapterOptions).resolveAlbum(albumId, { album, tracks });
}
